...
while ((sum(cellfun('isempty', defstr)) > 0) || ...
       (str2double(defstr{2}) < 2) || ...
       (sum(cellfun(@str2num, defstr(1:6)) < 0) > 0) || ...
       (str2double(defstr{3}) >= str2double(defstr{1})) || ...
...
%# Input variables:
box_length = str2double(defstr{1});
n_total = uint32(str2double(defstr{2}));
D_mean = str2double(defstr{3});
D_sdev = str2double(defstr{4});
o_f = str2double(defstr{5});
maxtrials = uint32(str2double(defstr{6}));
sortstr = defstr{7};
plotstr = defstr{8};
...
fi1 = fopen([result_dir,'Statistics.txt'], 'wt');
fprintf(fi1,'\n');
...
dlmwrite([result_dir,'Positions.txt'], P_mat(1:n_total,:), 'precision', 12);
dlmwrite([result_dir,'Radii.txt'], R_vec(1:n_total,:), 'precision', 12);

fi1 = fopen([result_dir,'Statistics.txt'], 'at');
fprintf(fi1,'\n');
fprintf(fi1,'\n');
...
switch (uschc)
  case 'Input script'
    ...
    [termflag] = m3d_inputgen(P_mat_ind, R_vec_ind, box_length, result_dir);
    if (termflag == 1)
  ...
  case 'Voxel mesh'
    ...
    while ((sum(cellfun('isempty', vxstr)) > 0) || ...
           (isscalar(str2double(vxstr{1})) == 0) || ...
           (sum(strcmp(vxstr{2}, {'C3D8', 'C3D8R', 'C3D20', 'C3D20R'})) == 0))
      warndlg('Please check input!', 'Warning');
      vxstr = inputdlg(vxvar, 'Mote3D voxel mesh generation', 1, vxdflt);
    end
    el_number = uint32(str2double(vxstr{1}));
    el_type = vxstr{2};
    disp('Generating voxel mesh...');
    [termflag] = m3d_vxmesh(P_mat_ind, R_vec_ind, box_length, el_number, el_type, result_dir);
    if (termflag == 1)
